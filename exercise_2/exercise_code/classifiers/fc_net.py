...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        grad_hidden_layer, grads['W2'], grads['b2'] = affine_backward(dscores, scores_cache)
        _, grads['W1'], grads['b1'] = affine_relu_backward(grad_hidden_layer, hidden_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
          w_index = 'W' + str(i)
          self.params[w_index] = np.random.normal(scale=weight_scale, size=(dim_layers[i-1], dim_layers[i]))
          
          b_index = 'b' + str(i)
          self.params[b_index] = np.zeros(dim_layers[i])
        
        # batch normalization
        if self.use_batchnorm:
          for i in range(1, self.num_layers):
            gamma_index = 'gamma' + str(i)
            beta_index = 'beta' + str(i)

            self.params[gamma_index] = np.ones(dim_layers[i])
            self.params[beta_index] = np.zeros(dim_layers[i])
    ...
    def loss(self, X, y=None):
        ...
        # softmax loss
        loss, grads_loss = softmax_loss(scores, y)

        # softmax loss with regularization
        loss_W = 0
        for i in range(1, self.num_layers+1):
          loss_W += np.sum(self.params['W' + str(i)]**2)
        loss += 0.5 * self.reg * loss_W

        # backpropagation output layer
        w_index = 'W' + str(self.num_layers)
        b_index = 'b' + str(self.num_layers)
        H_grads, grads[w_index], grads[b_index] = affine_backward(grads_loss, cache_list[-1])
        ...
        for i in range(self.num_layers - 1, 0, -1):
          w_index = 'W' + str(i)
          b_index = 'b' + str(i)
          H_grads, grads[w_index], grads[b_index] = affine_relu_backward(H_grads, cache_list[i - 1])
          grads[w_index] += self.reg * self.params[w_index]
