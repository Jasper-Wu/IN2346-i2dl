    ...
    def train(self, model, train_loader, val_loader, num_epochs=10, log_nth=0):
        ...
        for epoch in range(num_epochs):
            ...
            for i, (inputs, targets) in enumerate(train_loader, 1):
                # enumerate(train_loader, 1): assign the beginning index
            ...
            for inputs, targets in val_loader:
                ...
                # what is the difference with model(inputs) ???
                loss = self.loss_func(outputs, targets)
                val_losses.append(loss.data.cpu().numpy())

                _, preds = torch.max(outputs, 1)

                # only allow imaages/pixels with target >= 0 e.g. for segmentation
                targets_mask = targets >= 0
                scores = np.mean((preds == targets)[targets_mask].data.cpu().numpy())
                val_scores.append(scores)

            model.train()
            # why put model.train() here instead at the beginning ???
